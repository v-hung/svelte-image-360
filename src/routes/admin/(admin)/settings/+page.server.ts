...
import { existsSync, fstat, mkdirSync, statSync, unlinkSync } from "fs"
      ...
      for (var [key, value] of data.entries()) {
        if (value && ((value as File).size > 0 || value.length > 0)) {
          if (typeof value === 'string') {
            ...
            const fileOld = await db.setting.findFirst({
              where: {
                name: key
              }
            })

            if (fileOld && fileOld.value) {
              if (statSync(`.${fileOld.value}`)) {
                unlinkSync(`.${fileOld.value}`)
              }
            }
